...
```{r}
temp_df %>% plot_ly(x = ~ObservationDate, y = ~Confirmed_Cases, type = "scatter", mode = "line", name = "Confirmed Cases")  %>% add_trace(y = ~Recovered_Cases, name =
                         "Recovered Cases") %>% add_trace(y = ~Death_Cases, name =
                         "Deceased Cases") %>% layout(legend = list(x = 0.05, y = 0.9),
                                                      yaxis = list(title = "Number of Cases"))
```

Countries {data-orientation=columns}
=======================================================================
...
temp_df %>% plot_ly(x = ~Country.Region, y = ~Confirmed, type = "bar", color =
                      ~Country.Region)  %>% layout(showlegend = F,xaxis = list(title = "Country"), yaxis = list(title = "Confirmed Cases"))
```
